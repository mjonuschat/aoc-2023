...
def enclosed(m, visited):
    for y in range(len(m)):
        verticals = 0
        for x in range(len(m[y])):
            if (x, y) in visited:
                kind = m[y][x]
                directions = PIPES[kind]
                if "N" in directions:
                    verticals += 1
                continue
            m[y][x] = "." if verticals % 2 == 0 else "I"
    return "".join(chain(*m)).count("I")

def find_loop(m, override = None):
    start_x, start_y = find_start(m)
    if override:
      m[start_y][start_x] = override
    ...
    while queue:
        ...
        if loc in visited:
            continue
        visited[loc] = distance
        x, y = loc
        for connection in PIPES[m[y][x]]:
            nx, ny, reverse = DIRECTIONS[connection]
            if y + ny < 0 or y + ny > len(m):
                continue
            if x + nx < 0 or x + nx > len(m[y + ny]):
                continue
            dest = m[y + ny][x + nx]
            if dest not in PIPES:
                continue
            if reverse in PIPES[dest]:
                queue.append(((x + nx, y + ny), distance + 1))

    return visited, max(visited.values())

def day10_puzzle1():
    m = list(line.strip()) for line in INPUT.strip().splitlines()
    _, max_distance = find_loop(m)
    print(f"Max distance: {max_distance}")

def day10_puzzle2():
    m = list(line.strip()) for line in INPUT.strip().splitlines()
    visited, max_distance = find_loop(m, "L")
    area = enclosed(m, visited)
    print(f"Area: {area}")
