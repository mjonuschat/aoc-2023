...
for line in INPUT.strip().splitlines():
    if not line:
        continue
    if "=" not in line:
        direction = [0 if d == "L" else 1 for d in line.strip()]
        continue
...
def day8_puzzle1():
    cur = "AAA"
    for steps, dir in enumerate(itertools.cycle(direction), 1):
        cur = m[cur][dir]
        if cur == "ZZZ":
            break
...
def day8_puzzle2():
    cur = [l for l in m.keys() if l[-1]=='A']
    steps = [-1 for _ in cur]

    for i, dir in enumerate(itertools.cycle(direction), 1):
        cur = [m[c][dir] for c in cur]
        for j, c in enumerate(cur):
            if steps[j]<0 and c[-1]=='Z': 
                steps[j] = i
        if all(s>0 for s in steps): 
            break

    total = lcm(*[step for step in steps])
